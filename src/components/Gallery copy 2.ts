...
async function getDataJson() {
  const url = `https://www.googleapis.com/drive/v3/files?q='${ROOT_FOLDER_ID}' in parents and name='data.json' and trashed=false&key=${API_KEY}&fields=files(id,name)`;
  const res = await fetch(url);
  const data = await res.json();
  if (!data.files?.length) throw new Error("data.json not found");
  const fileId = data.files[0].id;
  const jsonUrl = `https://www.googleapis.com/drive/v3/files/${fileId}?alt=media&key=${API_KEY}`;
...
async function getCategories() {
  const [json, files] = await Promise.all([getDataJson(), listAssets()]);

  (window as any).categories = json.categories.map((cat: any) => {
    const mappedAssets = cat.assets
      .map((assetName: string) => {
        const f = files.find((file: any) => file.name === assetName);
        if (!f) return null;
        return { name: assetName, url: f.url, mimeType: f.mimeType };
      })
      .filter(Boolean);
    ...
    .flatMap((cat) =>
      cat.assets.map((a: any) =>
        a.mimeType.startsWith("video")
          ? `<video src="${a.url}" data-category="${cat.short_name}" class="gallery-thumb" autoplay loop muted playsinline></video>`
          : `<img src="${a.url}" data-category="${cat.short_name}" class="gallery-thumb" alt="${a.name}">`
      )
    )
    .join("");

  // Re-bind lightbox + filtering
  bindGalleryLogic();
}
  ...
  tabs.forEach((tab: any) => {
    tab.addEventListener("click", () => {
      ...
      thumbs.forEach((thumb: any) => {
        ...
        thumb.style.display =
          category === "all" || thumbCategory === category ? "block" : "none";
      });
